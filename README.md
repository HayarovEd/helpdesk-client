...
JWT не сохраняется в `localStorage` или cookies. Для восстановления чата после обновления страницы текущая сессия хранится в `sessionStorage` до закрытия вкладки; пароль и внешний проверочный токен в хранилище не записываются. Формат `login_from_app` вынесен в `src/api.ts`.
...
```json
{
  ...
  "device_name": "<user agent, максимум 100 символов>",
  "platform": "FCM"
}
```

В foreground уведомление показывается браузером сразу. В background его показывает Firebase service worker. Запрос разрешения выполняется при открытии чата; если браузер требует пользовательского действия, регистрация автоматически повторяется при первом клике или нажатии клавиши. Для production сайт должен работать через HTTPS; `localhost` разрешён браузерами для разработки. Firebase Web config и VAPID key являются публичными web-настройками и могут быть переопределены через `.env.local`.

Поле `token` нельзя обрезать: FCM-токены могут быть длиннее 100 символов. Backend должен хранить его в колонке длиной не менее 255 символов или типа `TEXT`.

Если чат открыт внутри iframe, браузер может запретить запрос разрешения. Внешняя страница должна добавить разрешение iframe:

```html
<iframe src="https://your-widget.example" allow="notifications"></iframe>
```

Надёжнее открывать чат в отдельной вкладке по HTTPS. Если разрешение ранее было заблокировано, его нужно включить в настройках уведомлений браузера для домена виджета.
